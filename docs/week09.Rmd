...
\ell\left(\bb ; \by, \bX \right) & = \sum_{i=1}^n \log\left(\frac{p_i}{1-p_i}\right) y_i + \log(1-p_i) \\
 & \propto \sum_{i=1}^n (\bx_i \bb) y_i - \log\left(1 + \exp\left(\bx_i \bb\right) \right)  
\end{aligned}
$$

## Iteratively Reweighted Least Squares

1. Initialize $\bb^{(1)}$ and set $p_i^{(1)} = \logit^{-1}\left( \bx_i \bb^{(1)} \right)$.
...
Next week we will study the summary statistics, sampling distributions, and inference procedures for GLMs that are analogous to those we covered above for least squares regression.
