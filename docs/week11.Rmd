...
<div id="left">
<span style="font-size:0.75em; line-height:0%">
$$
\begin{array}{rc}
& \bY_{m \times n} \\
& \text{observations} \\
...
<span style="font-size:0.65em; line-height:0%">
$$
\begin{array}{cccccc}
...
$$
\sum_{j=1}^n \left(y_j - \sum_{i=1}^m \beta_{i} x_{ij}\right)^2 + \lambda \sum_{k=1}^m \beta_i^2.
$$

Recall the $\ell_2$ norm: $\sum_{k=1}^m \beta_i^2 = \| \bb \|^2_2$.  Sometimes ridge regression is called $\ell_2$ penalized regression.
...
When $d > n$ or when there is high collinearity, then $(\bX\bX^T)^{-1}$ will not exist.  
...
$$
-\frac{1}{2\sigma^2}\sum_{j=1}^n \left(y_j - \sum_{i=1}^m \beta_{i} x_{ij}\right)^2 - \frac{\lambda}{2\sigma^2} \sum_{k=1}^m \beta_i^2
$$
...
$$
\sum_{j=1}^n \left(y_j - \sum_{i=1}^m \beta_{i} x_{ij}\right)^2 + \lambda \sum_{k=1}^m |\beta_i|.
$$

Note that $\sum_{k=1}^m |\beta_i| = \| \bb \|_1$, which is the $\ell_1$ vector norm.
...
$$
-\frac{1}{2\sigma^2}\sum_{j=1}^n \left(y_j - \sum_{i=1}^m \beta_{i} x_{ij}\right)^2 - \frac{\lambda}{2\sigma^2} \sum_{k=1}^m |\beta_i|
$$
